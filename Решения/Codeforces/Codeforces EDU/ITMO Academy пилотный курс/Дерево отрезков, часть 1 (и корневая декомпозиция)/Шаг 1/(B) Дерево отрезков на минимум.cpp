...
struct segtree {
	...
	int get_min(int l, int r, int x, int lx, int rx) {
		...
		int mid = (lx + rx) / 2;
		int leftmin = get_min(l, r, 2 * x + 1, lx, mid);
		int rightmin = get_min(l, r, 2 * x + 2, mid, rx);
...
int main() {
	...
	while (--m >= 0) {
		...
		else { // возврат минимума на полуинтервале [l, r)
			cin >> l >> r;
			cout << ST.get_min(l, r) << '\n';
		}
	}
