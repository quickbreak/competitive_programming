...
struct SegmentTree {
public:
	vector<std::pmr::multiset<pair<int, int>>>tree_;
	int size_ = 1;
	...
	void init(int n) {
		...
		tree_.assign(2 * size_ - 1, std::pmr::multiset<pair<int, int>>());
...
int main() {
    // создаём monotonic buffer resource
    std::pmr::monotonic_buffer_resource memory;
    // делаем его дефолтным для всех pmr структур
